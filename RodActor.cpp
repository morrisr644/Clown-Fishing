	...
	:Actor(game)
{
	...
	mCameraComp = game->GetPlayer()->GetCamera();

	mRodModel = new Actor(game);
	mRodModel->SetScale(0.75f);
	mMeshComp = new MeshComponent(mRodModel);
...
void RodActor::UpdateActor(float deltaTime)
{
	...
	mRodModel->SetPosition(modelPos);
	...
	mRodModel->SetRotation(q);
}

//void RodActor::ActorInput(const uint8_t* keys)
//{
...
void RodActor::SetVisible(bool visible)
{
	mMeshComp->SetVisible(visible);
}

void RodActor::FixCollisions()
{
